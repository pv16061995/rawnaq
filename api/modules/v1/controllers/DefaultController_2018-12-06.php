...
class DefaultController extends Controller {
    ...
    // Hotel
    public function actionGetAllDestination($limit, $show_on_home = 0, $page = 0) { 
		$with = [
            'destinationGalleries'
        ];

        $total_destinations = Destination::find()->where('hotel_destination.id NOT IN (:id1,:id2)', ['id1'=> 1, 'id2' => 8]);
        ...
        //try to display static page from datebase
        $destinations = Destination::getDb()->cache(function ($db) use($limit, $with, $show_on_home, $pagination) {
            if ($limit == -1) {
    ...
    public function actionGetAllHotels($slug, $limit, $search = '') {
        //try to display static page from database
        $with = [
            'destinationGalleries'
        ];
        $destination = Destination::getDb()->cache(function ($db) use($limit, $slug) {
        	// return Destination::findOne(['slug' => $slug]);
        	return Destination::find()->where('slug = "' . $slug . '"')->one();
            // if ($limit == -1) {
            //     return Destination::findOne()->where(['slug' => $slug]);
            // } else {
            //     return Destination::findOne()->where(['slug' => $slug])->limit($limit);
            // }
        }, 3600);
        //, 'status' => Hotel::STATUS_PUBLISHED]
        // print_r($destination->getDestinationGalleries()->all());
    ...
    public function actionSendQuote() {
        ...
        if(count($params) > 0){
            ...
            if(Yii::$app->mailer->compose()
                    ...
                    ->setBcc(['saad.5690@gmail.com', 'moni.7amody@gmail.com'])
