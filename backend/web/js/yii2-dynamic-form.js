    ...
    var methods = {
        ...
        addItem: function (widgetOptions, e, $elem) {
           ...
           if(widgetOptions.widgetContainer==='dynamicform_wrapper'){
                var lastIndex = $('.form-options-item').length-1;
                if(lastIndex<1)
                    return false;
                ...
                $('.form-options-item:eq('+lastIndex+') [role="media-launch"]').on("click", function (e) {
                    e.preventDefault();

                    var modal = $('.form-options-item:eq('+lastIndex+') [role="media-modal"]');
                    modal.attr('data-btn-id',$(this).attr("id"));
                    var frameContainer = modal.attr('data-frame-id').split('-');
                    var newFrameContainer = "";
                    for(var i=0; i<frameContainer.length;i++){
                        if(i===1){
                            newFrameContainer+= lastIndex;
                        }else{
                            newFrameContainer+= frameContainer[i];
                        }
                        if(i !== frameContainer.length-1){
                            newFrameContainer+="-";
                        }
                    }
                    modal.attr('data-frame-id',newFrameContainer);
                    var dataContainer = modal.attr('data-input-id').split('-');
                    var newDataContainer = "";
                    for(var i=0; i<dataContainer.length;i++){
                        if(i===1){
                            newDataContainer+= lastIndex;
                        }else{
                            newDataContainer+= dataContainer[i];
                        }
                        if(i !== dataContainer.length-1){
                            newDataContainer+="-";
                        }
                    }
                    modal.attr('data-input-id',newDataContainer);
                    
                    var imageContainer = modal.attr('data-image-container').split('-');
                    var newImageContainer = "";
                    for(var i=0; i<imageContainer.length;i++){
                        if(i===2){
                            newImageContainer+= lastIndex;
                        }else{
                            newImageContainer+= imageContainer[i];
                        }
                    ...
                    modal.attr('data-image-container',newImageContainer);
                    var iframe = $('<iframe src="' + modal.attr("data-frame-src")
